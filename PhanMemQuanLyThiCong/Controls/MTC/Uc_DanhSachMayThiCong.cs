...
{
    public partial class Uc_DanhSachMayThiCong : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void bgv_ListMay_InitNewRow(object sender, InitNewRowEventArgs e)
        {
            ...
            view.SetRowCellValue(e.RowHandle, bgv_LoaiMay, lstMayMoc.FirstOrDefault().Code);
            view.PostEditor();
            view.UpdateCurrentRow();
        }
        ...
        private void bgv_ListMay_PopupMenuShowing(object sender, DevExpress.XtraGrid.Views.Grid.PopupMenuShowingEventArgs e)
        {
            ...
            if (e.MenuType == GridMenuType.Row)
            {
                ...
                menuItemXoa.Tag = hitInfo.Column;
                e.Menu.Items.Add(menuItemXoa);
        ...
        {
            string PathSave = "";
            var saveDialog = SharedControls.saveFileDialog;
            saveDialog.DefaultExt = "xlsx";
            saveDialog.Filter = "Excel (*.xlsx) | *.xlsx";
            saveDialog.AddExtension = false;

            string time = DateTime.Now.ToString("dd-MM-yyyy_hh-mm-ss");
            saveDialog.FileName = $"Bảng danh sách máy thi công_{time}.xlsx";
            string fileName = "";
            if (saveDialog.ShowDialog() == DialogResult.OK)
            {
                PathSave = saveDialog.FileName;
                if (!PathSave.EndsWith(".xlsx"))
                    PathSave += ".xlsx";

                fileName = Path.GetFileName(PathSave);
            }
            else
                return;
            WaitFormHelper.ShowWaitForm("Đang phân tích dữ liệu Máy thi công");
            string m_Path = Path.Combine(BaseFrom.m_path, "Template", "FileExcel", "18.2 Bảng danh sách máy thi công.xlsx");
            SpreadsheetControl Spread = new SpreadsheetControl();
            Spread.LoadDocument(m_Path);
            Worksheet ws = Spread.Document.Worksheets["Danh sách máy"];

            string dbString = $"SELECT\r\n\tROW_NUMBER( ) OVER (ORDER BY SortID ) STT" +
    $",\r\n\tCode,\r\n\tTen,\r\n\tNo,\r\n\tTenTrangThai,\r\n\tGhiChu,\r\n\t NgayMuaMay,\r\n\tTenLoaiMay,\r\n\tTenVietTat," +
    $"\r\n\tBienSo,\r\n\tXuatXu,\r\n\tNamSanXuat,\r\n\tSoKhung,GiaMuaMay,GiaCaMay,ChiPhiSuaChua,ChiPhiKhac,TienTaiXe,ViTriMay," +
    $"\r\n\tDonVi,\r\n\tHaoPhi,\r\n\tCaMayKm,\r\n\tGROUP_CONCAT( DISTINCT CodeDuAn )" +
    $" AS DuAn,\r\n\tGROUP_CONCAT( DISTINCT TenChuSoHuu ) AS TenChuSoHuu,\r\n\tGROUP_CONCAT( DISTINCT TenDuAn ) AS TenDuAn,\r\n\tGROUP_CONCAT( DISTINCT TenNguoiVanHanh )" +
    $" AS TenNguoiVanHanh,\r\n\tGROUP_CONCAT( DISTINCT ( CASE WHEN LoaiNhienLieu = 1 THEN TenNhienLieu END ) )" +
    $" AS NhienLieuChinh,\r\n\tGROUP_CONCAT( DISTINCT ( CASE WHEN CaMayKm ='Ca' THEN '1 ca 7 tiếng' WHEN CaMayKm ='Km' THEN '100 Km' END ) ) AS DonViHaoPhi ," +
    $"\r\n\tGROUP_CONCAT( DISTINCT ( CASE WHEN LoaiNhienLieu = 2 THEN TenNhienLieu END ) )" +
    $" AS NhienLieuPhu " +
    $"FROM\r\n\tview_DanhSachMay \r\nGROUP BY\r\n\tCode ORDER BY \"SortId\" ASC";
            DataTable dtMay = DataProvider.InstanceTHDA.ExecuteQuery(dbString);
            Spread.BeginUpdate();
            var grLoaiThietBi = dtMay.AsEnumerable().GroupBy(x => x["TenLoaiMay"].ToString());
            int i = 8;
            Row LoaiThietBi = ws.Rows[7];
            ws.Rows[2]["B"].SetValueFromText($"Ngày xuất báo cáo: {DateTime.Now.ToString(MyConstant.CONST_DATE_FORMAT_SPSHEET)}");
            ws.Rows[3]["B"].SetValueFromText($"Người xuất báo cáo: {BaseFrom.BanQuyenKeyInfo.FullName}");
            int CTCha = 1;
            foreach(var LoaiTB in grLoaiThietBi)
            {
                Row Crow = ws.Rows[i++];
                ws.Rows.Insert(i+1, 1, RowFormatMode.FormatAsNext);
                Crow.CopyFrom(LoaiThietBi, PasteSpecial.All);
                Crow.Visible = true;
                int STT = 1;
                Crow[(int)MyConstant.LstBaoCaoMay.TenNguoiVanHanh].SetValueFromText(LoaiTB.Key.ToUpper());
                Crow[(int)MyConstant.LstBaoCaoMay.STT].SetValue(CTCha++);
                var grTenMay = LoaiTB.GroupBy(x => x["Ten"].ToString());
                foreach(var TenMay in grTenMay)
                {
                    Crow = ws.Rows[i++];
                    ws.Rows.Insert(i+1, 1, RowFormatMode.FormatAsNext);
                    var TenMayDefault = TenMay.FirstOrDefault();
                    Crow[(int)MyConstant.LstBaoCaoMay.STT].SetValue(STT++);
                    Crow[(int)MyConstant.LstBaoCaoMay.TenNguoiVanHanh].SetValueFromText(TenMayDefault["TenNguoiVanHanh"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.Ten].SetValueFromText(TenMay.Key);
                    Crow[(int)MyConstant.LstBaoCaoMay.BienSo].SetValueFromText(TenMayDefault["BienSo"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.No].SetValueFromText(TenMayDefault["No"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.DonVi].SetValueFromText(TenMayDefault["DonVi"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.TenChuSoHuu].SetValueFromText(TenMayDefault["TenChuSoHuu"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.XuatXu].SetValueFromText(TenMayDefault["XuatXu"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.NamSanXuat].SetValueFromText(TenMayDefault["NamSanXuat"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.SoKhung].SetValueFromText(TenMayDefault["SoKhung"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.GiaMuaMay].SetValue(TenMayDefault["GiaMuaMay"]);
                    Crow[(int)MyConstant.LstBaoCaoMay.GiaCaMay].SetValue(TenMayDefault["GiaCaMay"]);
                    Crow[(int)MyConstant.LstBaoCaoMay.NhienLieuChinh].SetValueFromText(TenMayDefault["NhienLieuChinh"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.HaoPhi].SetValue(TenMayDefault["HaoPhi"]);
                    Crow[(int)MyConstant.LstBaoCaoMay.ChiPhiSuaChua].SetValue(TenMayDefault["ChiPhiSuaChua"]);
                    Crow[(int)MyConstant.LstBaoCaoMay.ChiPhiKhac].SetValue(TenMayDefault["ChiPhiKhac"]);
                    Crow[(int)MyConstant.LstBaoCaoMay.TienTaiXe].SetValue(TenMayDefault["TienTaiXe"]);
                    Crow[(int)MyConstant.LstBaoCaoMay.CaMayKm].SetValueFromText(TenMayDefault["CaMayKm"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.NgayMuaMay].SetValueFromText(TenMayDefault["NgayMuaMay"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.ViTriMay].SetValueFromText(TenMayDefault["ViTriMay"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.TenTrangThai].SetValueFromText(TenMayDefault["TenTrangThai"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.TenDuAn].SetValueFromText(TenMayDefault["TenDuAn"].ToString());
                    Crow[(int)MyConstant.LstBaoCaoMay.GhiChu].SetValueFromText(TenMayDefault["GhiChu"].ToString());
                }
            }
            Spread.EndUpdate();
            Spread.Document.History.IsEnabled = true;
            Spread.SaveDocument(PathSave, DocumentFormat.Xlsx);
            WaitFormHelper.CloseWaitForm();
            MessageShower.ShowInformation("Xuất File thành công!");
            DialogResult dialogResult = XtraMessageBox.Show($"[{fileName}] thành công. Bạn có muốn mở file không???", "QUẢN LÝ THI CÔNG - THÔNG BÁO", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (dialogResult == DialogResult.Yes)
            {
                Process.Start(Path.Combine(PathSave, PathSave));
            }
        }

        private void sb_DocDanhSachMay_Click(object sender, EventArgs e)
        {
            ...
            args.Showing += Args_Showing_DocMay;
            DevExpress.XtraEditors.XtraMessageBox.Show(args);
        }
        private void Args_Showing_DocMay(object sender, XtraMessageShowingArgs e)
        {
                        ...
                        case ("OK"):
                            ...
                            button.Click += (ss, ee) =>
                            {
                                ...
                                if (rs == DialogResult.OK)
                                {
                                    ...
                                    if(!Spread.Document.Worksheets.Select(x=>x.Name).ToList().Contains("Danh sách máy"))
                                    {
                                        MessageShower.ShowError("Mẫu đọc vào không phải mẫu danh sách máy của hệ thống, Vui lòng chọn lại Mẫu khác!!!!");
                                        return;
                                    }
                                    Worksheet ws = Spread.Document.Worksheets["Danh sách máy"];
                                    Dictionary<string, string> Name = MyFunction.fcn_getDicOfColumn(ws.Range["Data"]);
                                    List<string> lstColum=Enum.GetValues(typeof(MyConstant.LstBaoCaoMay)).Cast<MyConstant.LstBaoCaoMay>().Select(v => v.ToString()).ToList();
                                    List<string> lstColumDic = Name.Select(x => x.Key).ToList();
                                    if (lstColumDic.Intersect(lstColum).Count() != lstColum.Count())
                                    {
                                        MessageShower.ShowError("Mẫu đọc vào đã chỉnh sửa số cột hoặc vị trí cột mặc định, Vui lòng chọn lại Mẫu khác!!!!");
                                        return;

                                    }
                                    int LastLine = ws.Range["Data"].BottomRowIndex;
                                    int begin = ws.Range["Data"].TopRowIndex;
                                    List<string> LoaiMayMoc = lstMayMoc.Select(x => x.Ten).ToList();
                                    string dbString = string.Empty;
                                    List <MTC_DanhSachMay> lstDanhSachMay = new List<MTC_DanhSachMay>();
                                    
                                    for (int i = begin; i <= LastLine; i++)
                                    {
                                        Row Crow = ws.Rows[i];
                                        if (!int.TryParse(Crow[Name["STT"]].Value.ToString(), out int STT))
                                            break;
                                        CellRange CellMerge = ws.Range[$"{Crow[Name["TenNguoiVanHanh"]].GetReferenceA1()}:{Crow[Name["GhiChu"]].GetReferenceA1()}"];
                                        string CodeLoaiMay = "";
                                        if (CellMerge.IsMerged)
                                        {
                                            string NameLoaiMay = Crow[Name["TenNguoiVanHanh"]].Value.TextValue;
                                            if (!LoaiMayMoc.Contains(NameLoaiMay))
                                            {
                                                CodeLoaiMay = Guid.NewGuid().ToString();
                                                dbString = $"INSERT INTO Tbl_MTC_LoaiMay (\"Code\",\"Ten\",\"SortId\") VALUES ('{CodeLoaiMay}','{NameLoaiMay}','{lstMayMoc.Count()+1}')";
                                                DataProvider.InstanceTHDA.ExecuteNonQuery(dbString);
                                            }
                                            else
                                                CodeLoaiMay = lstMayMoc.Where(x => x.Ten == NameLoaiMay).FirstOrDefault().Code;
                                            continue;
                                        }
                                        else
                                        {
                                            string TrangThai= Crow[Name["TenTrangThai"]].Value.TextValue;
                                            string CodeTrangThai = string.Empty;
                                            if (!string.IsNullOrEmpty(TrangThai))
                                            {
                                                List<MTC_TrangThai> NewTrangThai = lstTrangThais.Where(x => x.Ten == TrangThai).ToList();
                                                if (NewTrangThai.Any())
                                                    CodeTrangThai = NewTrangThai.FirstOrDefault().Code;
                                                else
                                                {
                                                    CodeTrangThai = Guid.NewGuid().ToString();
                                                    dbString = $"INSERT INTO Tbl_MTC_TrangThai (\"Code\",\"Ten\",\"SortId\") VALUES ('{CodeTrangThai}','{TrangThai}','{lstTrangThais.Count}')";
                                                    DataProvider.InstanceTHDA.ExecuteNonQuery(dbString);
                                                }
                                            }
                                            MTC_DanhSachMay item = new MTC_DanhSachMay();
                                            item.Ten = Crow[Name["Ten"]].Value.TextValue;
                                            item.TrangThai = CodeTrangThai;
                                            item.BienSo = Crow[Name["BienSo"]].Value.TextValue;
                                            item.TenVietTat = Crow[Name["TenVietTat"]].Value.TextValue;
                                            item.XuatXu = Crow[Name["XuatXu"]].Value.TextValue;
                                            item.NamSanXuat = Crow[Name["NamSanXuat"]].Value.TextValue;
                                            item.SoKhung = Crow[Name["SoKhung"]].Value.TextValue;
                                            item.GiaMuaMay =(long)Crow[Name["GiaMuaMay"]].Value.NumericValue;
                                            item.GiaCaMay = (long)Crow[Name["GiaCaMay"]].Value.NumericValue;
                                            item.ChiPhiSuaChua = (long)Crow[Name["ChiPhiSuaChua"]].Value.NumericValue;
                                            item.ChiPhiKhac = (long)Crow[Name["ChiPhiKhac"]].Value.NumericValue;
                                            item.TienTaiXe = (long)Crow[Name["TienTaiXe"]].Value.NumericValue;
                                            item.CaMayKm = Crow[Name["CaMayKm"]].Value.TextValue.ToUpper() == "KM" ? "Km" : "Ca";
                                            item.ViTriMay = Crow[Name["ViTriMay"]].Value.TextValue;
                                            item.DonVi= Crow[Name["DonVi"]].Value.TextValue;
                                            item.NgayMuaMay= Crow[Name["NgayMuaMay"]].Value.IsEmpty?DateTime.Now:Crow[Name["NgayMuaMay"]].Value.DateTimeValue;
                                            item.GhiChu = Crow[Name["GhiChu"]].Value.TextValue;
                                            lstDanhSachMay.Add(item);
                                        }
                                    }
                                    int res = DataProvider.InstanceTHDA.AddOrUpdate<MTC_DanhSachMay>("INSERT_DANHSACHMAY", lstDanhSachMay, false, true, true);
                                    LoadData();
                                    InitDataSoureControl();
                                    InitEventControl();
                                }
