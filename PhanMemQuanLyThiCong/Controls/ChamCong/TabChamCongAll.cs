...
{
    public partial class TabChamCongAll : DevExpress.XtraEditors.XtraUserControl
    {
        Uc_TenNhanVien m_ucTenNV = new Uc_TenNhanVien();
        public TabChamCongAll()
        {
            InitializeComponent();
...
$"'{m_ucTenNV._Code}')";
        ...
        private void Fcn_LoadTamUng()
        {
            ...
            if (RangeData.RowCount - 2 <= Dt_TamUng.Count() - 1)
        ...
        private void spsheet_TamUng_CellValueChanged(object sender, SpreadsheetCellEventArgs e)
        {
            ...
            if (colum == "TenNhanVien" || colum == "MaNhanVien" || colum == "ChucVu" || colum == "LoaiNhanVien"|| colum == "CodeDVPH"|| colum == "DuAn")
                return;
            if (RangeData.BottomRowIndex - 2 <= e.RowIndex)
        ...
        public void xtraTabControl_BangNhanCong_SelectedPageChanged(object sender, DevExpress.XtraTab.TabPageChangedEventArgs e)
        {
            ...
            {
                if (SharedControls.slke_ThongTinDuAn.EditValue is null && xtraTabControl_BangNhanCong.SelectedTabPage == xtraTabPage_CongNhat)
                {
                    MessageShower.ShowWarning("Vui lòng Chọn dự án!!!!!");
...
$"('{codeCongTacThuCong}','{e.Value}','{SharedControls.cbb_DBKH_ChonDot.SelectedValue}','{DVTH.Code}','{2}')";
        ...
        private void spsheet_TamUng_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Delete)
            {
                ...
                DuAnHelper.DeleteDataRows(DanhSachNhanVienConstant.TBL_CHAMCONG_TAMUNG, new string[] { Code });
        ...
        private void xtraTabControl_BangNhanCong_SelectedPageChanging(object sender, DevExpress.XtraTab.TabPageChangingEventArgs e)
        {
            if (SharedControls.slke_ThongTinDuAn is null)
                return;
            if (e != null && SharedControls.slke_ThongTinDuAn.EditValue is null && e.Page == xtraTabPage_CongNhat)
                e.Cancel = true;
        }
